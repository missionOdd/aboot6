export default {
	methods: {
		...
		downloadList() {
		  this.$http_json({
		    url: "?sort=createdAt,desc<#if queryColumns??>this.selectType ? this.selectType=this.searchVal : ""<#else>this.searchVal ? title=this.searchVal : ""</#if><#if betweens??>
		      this.date
		...
		getList(page, size) {
		  this.$http_normal({
		    url: "?page=page - 1size=
		      size
		      sort=createdAt,desc<#if queryColumns??>this.selectType ? this.selectType=this.searchVal : ""<#else>this.searchVal ? title=this.searchVal : ''</#if><#if betweens??>
		      this.date
		        ? greatTime=this.dateArray[0] lessTime=this.dateArray[1]
		        : ''
		      }</#if>",
